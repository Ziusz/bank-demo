            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="mb-4">
                        ...
                        <p class="text-2xl font-bold text-green-600">${{ number_format($balance, 2) }}</p>
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($transactions as $transaction)
                                <tr>
                                    ...
                                    @if (Auth::id() == $transaction->recipient_id)
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 dark:text-green-600">
                                        +${{ number_format($transaction->amount, 2) }}
                                    </td>
                                    @else
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-red-600 dark:text-red-600">
                                        -${{ number_format($transaction->amount, 2) }}
                                    </td>
                                    @endif                                  
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
